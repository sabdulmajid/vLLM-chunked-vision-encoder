...
async def _sample_metrics(
    ...
    *,
    ...
    process = psutil.Process(server_pid) if server_pid is not None else None
...
async def _run_remote_openai_benchmark(
    *,
    ...
    control = await _measure_streaming_ttft(base_url=control_base_url, payload=control_payload)
    experimental = await _measure_streaming_ttft(
        base_url=experimental_base_url,
        payload=experimental_payload,
    )
    improvement_ms = control.ttft_ms - experimental.ttft_ms
    return BenchmarkComparison(
        control=control,
